...
Актуальные runtime-флаги, безопасный cleanup и статус приемки на 03.09.2026:
`docs/GLOBAL_USERS_ROLLOUT_RUNBOOK.md`. Полные отчеты ниже содержат персональные
данные: хранить их защищенно, не коммитить. Для обычного контроля использовать
`global-users:backfill:summary` и `verify:global-links:summary`.
