...
const DirectionCardButtons = ({
  ...
  const rule = loggedUserActiveRole?.generalPage?.directions
  ...
  const canMove = !forForm && loggedUserActiveRole?.generalPage?.directions
  ...
  if (!customOnly) {
    ...
    if (canEdit && item.status !== 'closed') {
      buttons.push({
        ...
        icon: faPencilAlt,
        onClick: () => {
          modalsFunc.direction.edit(item._id)
        },
        color: 'orange',
    ...
    if (canClone) {
      buttons.push({
        ...
        icon: faCopy,
        onClick: () => {
          modalsFunc.direction.add(item._id, itemProps)
        },
        color: 'blue',
    ...
    if (canDelete && item.status !== 'closed') {
      buttons.push({
        ...
        icon: faTrashAlt,
        onClick: () => {
          modalsFunc.direction.delete(item._id)
        },
        color: 'red',
        tooltipText: 'Удалить',
      })
...
export default DirectionCardButtons
